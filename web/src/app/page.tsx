...
interface CorrelationData {
  ...
  high_corr_warnings: { pair_a: string; pair_b: string; correlation: number; extreme: boolean }[];
...
function CorrelationPanel({ data, quote = "USDC" }: { data: CorrelationData; quote?: string }) {
  const { matrix, pairs, portfolio_var_pct, portfolio_var_abs, high_corr_warnings, size_adjustments } = data;
    ...
    <div className="card p-4 sm:p-5 fade-in">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-bold font-mono ${portfolio_var_pct > 5 ? "text-[var(--down)]" : portfolio_var_pct > 3 ? "text-[var(--warn)]" : "text-[var(--up)]"}`}>
      ...
      {n >= 2 && matrix.length >= 2 && (
        <div className="mb-3">
          <div className="inline-grid gap-px rounded overflow-hidden" style={{
            gridTemplateColumns: `48px repeat(${n}, 1fr)`,
            background: "var(--border-subtle)",
          }}>
            <div className="bg-[var(--bg-primary)] p-1" />
            {coins.map(c => (
              <div key={`h-${c}`} className="bg-[var(--bg-primary)] px-2 py-1.5 text-center text-[9px] font-bold text-[var(--text-tertiary)]">{c}</div>
            ))}
            {matrix.map((row, i) => {
              const frag = [
                <div key={`l-${i}`} className="bg-[var(--bg-primary)] px-2 py-1.5 text-[9px] font-bold text-[var(--text-tertiary)] flex items-center">{coins[i]}</div>,
                ...row.map((v, j) => (
                  <div key={`c-${i}-${j}`}
                    className="px-2 py-1.5 text-center text-[11px] font-mono font-semibold transition-colors"
                    style={{ background: i === j ? "var(--bg-secondary)" : corrBg(v), color: i === j ? "var(--text-quaternary)" : corrColor(v) }}>
          ...
          <div className="flex items-center gap-3 mt-1.5 text-[8px] text-[var(--text-quaternary)]">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm" style={{ background: "color-mix(in srgb, var(--up) 30%, transparent)" }} />niedrig</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm" style={{ background: "color-mix(in srgb, var(--warn) 30%, transparent)" }} />hoch (&gt;0.7)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm" style={{ background: "color-mix(in srgb, var(--down) 30%, transparent)" }} />extrem (&gt;0.9)</span>
          </div>
        </div>
      )}
      ...
      {high_corr_warnings.length > 0 && (
        <div className="space-y-1 mb-2">
          {high_corr_warnings.map((w, i) => (
            <div key={i} className="flex items-center gap-1.5 px-2 py-1 rounded text-[9px] font-medium"
              style={{
      ...
      {Object.keys(size_adjustments).length > 0 && (
        <div className="flex flex-wrap gap-2 text-[9px] text-[var(--text-quaternary)]">
          {Object.entries(size_adjustments).map(([p, factor]) => (
            <span key={p}>
              {p.split("/")[0]}: <strong className="text-[var(--warn)]">−{((1 - factor) * 100).toFixed(0)}%</strong> Size
            </span>
          ))}
        </div>
      )}
      ...
      {n < 2 && (
        <p className="text-[9px] text-[var(--text-quaternary)]">Korrelationsmatrix verfuegbar ab 2 Pairs</p>
      )}
    </div>
  );
}
...
interface MCResult {
  median_pnl: number; mean_pnl: number; std_pnl: number;
  percentile_5: number; percentile_1: number;
  max_loss: number; best_case: number;
  probability_of_loss: number;
  value_at_risk_95: number; conditional_var_95: number;
  mean_max_drawdown: number; mean_trades: number;
  distribution: number[];
  n_simulations: number; days_forward: number;
}
...
function MonteCarloPanel() {
  ...
  const runMC = async () => {
    ...
    try {
      ...
      if (!res.ok) { setError("Start fehlgeschlagen"); setLoading(false); return; }
      ...
      for (let i = 0; i < 60; i++) {
        ...
        if (data.status === "completed" && data.result) {
          setResult(data.result as MCResult); setLoading(false); return;
        }
        if (data.status === "failed") { setError("Simulation fehlgeschlagen"); setLoading(false); return; }
      }
      setError("Timeout"); setLoading(false);
  ...
  return (
    <div className="rounded-lg p-2.5 mb-3" style={{ background: "var(--bg-secondary)" }}>
      <div className="flex items-center justify-between mb-2">
        <div className="text-[8px] text-[var(--text-quaternary)] uppercase tracking-wider font-semibold">Monte-Carlo Stress-Test</div>
        <button onClick={runMC} disabled={loading}
          className="px-2 py-1 rounded text-[8px] font-semibold transition-all active:scale-95 disabled:opacity-50"
          style={{ background: "color-mix(in srgb, var(--accent) 12%, transparent)", color: "var(--accent)", border: "1px solid color-mix(in srgb, var(--accent) 20%, transparent)" }}>
          {loading ? "Laeuft..." : "Starten"}
        </button>
      </div>
      {error && <div className="text-[9px] text-[var(--down)] mb-1">{error}</div>}
      {result && (
        <div className="space-y-2">
          <div className="grid grid-cols-3 gap-1.5 text-[9px]">
            <div className="text-center">
              <div className="text-[7px] text-[var(--text-quaternary)] uppercase">VaR 95%</div>
              <div className="font-mono font-bold text-[var(--down)]">{fmt(result.value_at_risk_95)}</div>
            </div>
            <div className="text-center">
              <div className="text-[7px] text-[var(--text-quaternary)] uppercase">CVaR 95%</div>
              <div className="font-mono font-bold text-[var(--down)]">{fmt(result.conditional_var_95)}</div>
            </div>
            <div className="text-center">
              <div className="text-[7px] text-[var(--text-quaternary)] uppercase">P(Verlust)</div>
              <div className="font-mono font-bold" style={{ color: result.probability_of_loss > 50 ? "var(--down)" : "var(--up)" }}>{result.probability_of_loss.toFixed(0)}%</div>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-1.5 text-[9px]">
            <div className="text-center">
              <div className="text-[7px] text-[var(--text-quaternary)] uppercase">Median</div>
              <div className={`font-mono font-bold ${result.median_pnl >= 0 ? "text-[var(--up)]" : "text-[var(--down)]"}`}>{fmt(result.median_pnl)}</div>
            </div>
            <div className="text-center">
              <div className="text-[7px] text-[var(--text-quaternary)] uppercase">Best</div>
              <div className="font-mono font-bold text-[var(--up)]">{fmt(result.best_case)}</div>
            </div>
            <div className="text-center">
              <div className="text-[7px] text-[var(--text-quaternary)] uppercase">Worst</div>
              <div className="font-mono font-bold text-[var(--down)]">{fmt(result.max_loss)}</div>
            </div>
          </div>
          {result.distribution.length > 5 && (() => {
            const d = result.distribution;
            const bins = 20;
            const mn = d[0]; const mx = d[d.length - 1];
            const step = (mx - mn) / bins || 1;
            const counts = new Array(bins).fill(0);
            for (const v of d) { const b = Math.min(Math.floor((v - mn) / step), bins - 1); counts[b]++; }
                ...
                {counts.map((c, i) => (
                  <div key={i} className="flex-1 rounded-t-sm transition-all"
                    style={{
                      height: `${(c / maxC) * 100}%`,
                      minHeight: c > 0 ? 2 : 0,
                      background: i < zeroIdx ? "var(--down)" : i === zeroIdx ? "var(--warn)" : "var(--up)",
                      opacity: 0.7,
                    }}
          ...
          <div className="text-[7px] text-[var(--text-quaternary)] text-center">
            95%-Konfidenz: Verlust max. ${Math.abs(result.value_at_risk_95).toFixed(2)} in {result.days_forward}d ({result.n_simulations} Sims)
          </div>
        </div>
      )}
    </div>
